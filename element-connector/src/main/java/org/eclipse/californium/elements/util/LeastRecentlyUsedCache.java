...
/**
 * 具有最大容量的内存缓存，并支持基于LRU策略驱逐陈旧条目。
 *
 * 缓存会自动跟踪值的上次访问时间。每次从存储中读取或放入值时，都会更新访问时间。
 *
 * 如果满足以下任一条件，则可以将值成功添加到缓存中：
 * - 缓存的剩余容量大于零。
 * - 高速缓存包含至少一个陈旧条目，即至少在高速缓存的到期阈值时段内未被访问的条目。
 * 在这种情况下，最近最少访问的陈旧条目从高速缓存中逐出，以便为要添加的新值腾出空间。
 *
 * 此实现使用一个java.util.HashMap作为其后备存储。除此之外，缓存还以访问时间顺序保存条目的双向链表列表
 *
 * 访问缓存的条目（例如put，get，remove）不是线程安全的。客户端应显式序列化从多个线程对缓存的访问。
 *
 * 插入，查找和删除条目在 O（log n）中完成。
 *
 * @param <K> 缓存中使用的密钥类型。
 * @param <V> 缓存中使用的值的类型。
 */
public class LeastRecentlyUsedCache<K, V> {
	 ...
	 */
	/**
	 * 创建初始容量为{@link #DEFAULT_INITIAL_CAPACITY}，最大容量为
	 * {@link #DEFAULT_CAPACITY}，过期时间阈值为{@link #DEFAULT_THRESHOLD_SECS}秒
	 * 的缓存
	 */
	 ...
	 * 从给定的配置参数中创建一个cache
	 *
	 * @param initialCapacity 缓存初始化以支持的初始条目数。每次使用0.75％的当前容量时，
	 *                           缓存的容量将动态增加一倍，但永远不会超过maxCapacity
	 * @param maxCapacity 缓存可以管理的最大条目数
