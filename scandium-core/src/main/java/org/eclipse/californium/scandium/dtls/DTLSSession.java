...
public final class DTLSSession {
	 ...
	 * 当前的resd epoch，每当收到一个CHANGE_CIPHER_SPEC消息的时候会加一
	 */
	 ...
	 */
	/**
	 * 检查在当前epoch期间是否已收到给定记录。
	 *
	 * 检查是根据DTLS 1.2规范4.1.2.6节中描述的滑动窗口完成的
	 * @param sequenceNo
	 * @return
	 */
	 ...
	 *
	 * 将记录标记为已被接收，以便如果再次接收该记录，则可将其检测为重复，
	 * 例如，如果客户端重新发送记录，因为它会进入超时状态。
	 *
	 * 仅当记录属于此会话的当前read epoch的时候，这个record才会被标记为已接收
	 *
	 * @param epoch the record's epoch
	 * @param sequenceNo the record's sequence number
	 */
