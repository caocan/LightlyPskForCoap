...
/**
 * 一个在内存中的PSK密钥库
 * 如果你不需要初始化连接，那么可以使用{@link #setKey(String, byte[])}来添加identity/key
 * 如果需要初始化连接，那么可以使用{@link #addKnownPeer(InetSocketAddress, String, byte[])}来添加已知的对等点
 *
 * 这个PSK密钥库主要用于测试，你应该将密钥以一种更加安全的方式来进行存储
 * 在内存中存储不是一个好办法
 */
